...
tap.test('excessively deep subfolder nesting', function (t) {
  var tf = path.resolve(__dirname, 'resources', 'excessively-deep.tar')
  var data = fs.readFileSync(tf)
  var warnings = []
  var Entry = require('../lib/entry.js')

  var check = function (t, maxDepth) {
    if (typeof maxDepth === 'undefined') maxDepth = 1024
    t.match(warnings, [
      ['TAR_ENTRY_ERROR',
  ...
  t.test('async', function (t) {
    ...
    // Listen to 'warn' event instead of using onwarn callback
    extract.on('warn', function (code, message, data) {
  ...
  t.test('sync', function (t) {
    var extractDir = path.resolve(__dirname, 'tmp', 'excessively-deep-test-sync')
    require('rimraf').sync(extractDir)
    warnings.length = 0
    var extract = tar.Extract({
      path: extractDir
  ...
  t.test('async set md', function (t) {
    var extractDir = path.resolve(__dirname, 'tmp', 'excessively-deep-test-md')
    require('rimraf').sync(extractDir)
    warnings.length = 0
    var extract = tar.Extract({
      path: extractDir,
  ...
  t.test('sync set md', function (t) {
    var extractDir = path.resolve(__dirname, 'tmp', 'excessively-deep-test-sync-md')
    require('rimraf').sync(extractDir)
    warnings.length = 0
    var extract = tar.Extract({
      path: extractDir,
