...
tap.test('excessively deep subfolder nesting', function (t) {
  ...
  var check = function (t, maxDepth) {
    maxDepth = maxDepth || 1024
    t.ok(warnings.length > 0, "should have warnings")
    var found = false
    for (var i = 0; i < warnings.length; i++) {
      if (warnings[i][0] === 'TAR_ENTRY_ERROR' &&
        warnings[i][1] === 'path excessively deep') {
        found = true
        t.ok(warnings[i][2].depth > maxDepth, "depth should exceed maxDepth")
        t.equal(warnings[i][2].maxDepth, maxDepth, "maxDepth should match")
        t.ok(warnings[i][2].entry instanceof Entry, "entry should be Entry instance")
        t.ok(typeof warnings[i][2].path === 'string', "path should be string")
        break
      }
    }
    t.ok(found, "should have path excessively deep warning")
    warnings.length = 0
    t.end()
  }

  t.test('async', function (t) {
    var cwd = path.resolve(__dirname, "tmp/extract-deep-test")
  ...
  t.test('async set md', function (t) {
    var cwd = path.resolve(__dirname, "tmp/extract-deep-test2")
    try { fs.mkdirSync(cwd) } catch (e) { }
    var extract = Extract({
      path: cwd,
      onwarn: onwarn,
