...
function Extract(opts) {
  ...
  me.on("entry", function (entry) {
    // check path depth before processing
    ...
    if (isFinite(me.maxDepth) && parts.length > me.maxDepth) {
      ...
      if (opts.onwarn) {
        opts.onwarn('TAR_ENTRY_ERROR', 'path excessively deep', warnData)
      }
      me.warn('TAR_ENTRY_ERROR', 'path excessively deep', warnData)
      entry.abort()
      // Pause the entry to prevent it from flowing to _fst
      entry.pause()
      // Destroy the entry to ensure it doesn't get processed
      entry.destroy()
      return  // Exit early - don't process this entry at all
    }
    // if there's a "strip" argument, fthen strip off that many
    // path components.
    if (opts.strip) {
      ...
      parts = parts.slice(opts.strip) // .join("/")
      entry.path = entry.props.path = parts.join("/") // parts
      if (entry.linkpath) {
    ...
    // check if path is excessively deep
    if (isFinite(me.maxDepth) && parts.length > me.maxDepth) {
      me.warn('TAR_ENTRY_ERROR', 'path excessively deep', {
        entry: entry,
        path: entry.path,
        depth: parts.length,
        maxDepth: me.maxDepth,
