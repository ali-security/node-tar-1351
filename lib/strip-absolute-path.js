...
module.exports = function stripAbsolutePath(pathStr) {
    var r = ''
    while (pathStr && isAbsolute(pathStr)) {
        // windows will think that //x/y/z has a "root" of //x/y/
        var root = pathStr.charAt(0) === '/' ? '/' : parse(pathStr).root
        // Safety check: if root is empty, break to avoid infinite loop
        if (!root || root.length === 0) {
            break
        }
        // If root is the entire path, strip it and we're done
        if (root.length >= pathStr.length) {
            r += root
            pathStr = ''
            break
        }
