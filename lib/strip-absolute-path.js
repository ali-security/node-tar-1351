...
function getRoot(p) {
    ...
    if (p.charAt(0) === '/' && p.charAt(1) === '/') {
        // Check for //?/X:/ pattern (extended-length path)
        // Need at least 6 characters for //?/X:, 7 for //?/X:/
        // Pattern: //?/X:/ where X is a drive letter
        if (p.length >= 6 && p.charAt(2) === '?' && p.charAt(3) === '/') {
            // Check if we have a drive letter pattern (X:) at position 4-5
            var driveLetter = p.charAt(4)
            var colon = p.charAt(5)
            if (driveLetter && colon === ':' && /^[a-zA-Z]$/.test(driveLetter)) {
                // Check if there's a / after the drive letter (X:)
                // For //?/X:/y/z, charAt(6) should be '/'
                if (p.length >= 7 && p.charAt(6) === '/') {
                    return p.substr(0, 7) // //?/X:/
                }
                // If no / after drive letter, return //?/X:
                return p.substr(0, 6) // //?/X:
            }
        }
        return '//'
