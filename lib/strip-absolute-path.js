...
function getRoot(p) {
    ...
    if (p.charAt(0) === '/' && p.charAt(1) === '/') {
        ...
        // Need at least 7 characters: //?/X:/
        if (p.length >= 7 && p.charAt(2) === '?' && p.charAt(3) === '/' &&
            p.charAt(4) && p.charAt(5) === ':' && p.charAt(6) === '/') {
            // Check if charAt(4) is a letter (drive letter)
            var driveLetter = p.charAt(4)
            var driveLetterCode = driveLetter.charCodeAt(0)
            if ((driveLetterCode >= 65 && driveLetterCode <= 90) || // A-Z
                (driveLetterCode >= 97 && driveLetterCode <= 122)) { // a-z
                return p.substr(0, 7) // //?/X:/
            }
        }
        // Also handle //?/X: without trailing slash (at least 6 chars)
        if (p.length >= 6 && p.charAt(2) === '?' && p.charAt(3) === '/' &&
            p.charAt(4) && p.charAt(5) === ':') {
            var driveLetter = p.charAt(4)
            var driveLetterCode = driveLetter.charCodeAt(0)
            if ((driveLetterCode >= 65 && driveLetterCode <= 90) || // A-Z
                (driveLetterCode >= 97 && driveLetterCode <= 122)) { // a-z
