...
Pack.prototype._process = function () {
  ...
  // strip absolute paths more comprehensively (handles multiple roots)
  // absolutes on posix are also absolutes on win32
  // so we only need to test this one to get both
  var originalPath = wprops.path
  var s = stripAbsolutePath(originalPath)
  if (s[0]) {
    wprops.path = s[1]
    if (me.warn) {
      me.warn('stripping ' + s[0] + ' from absolute path', originalPath)
    }
  }
