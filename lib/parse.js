...
Parse.prototype._startEntry = function (c) {
  ...
  // Check return values from event listeners
  // If any listener returns false, abort the entry
  // We manually call listeners to check return values (replaces emit)
  var listeners = this.listeners(ev)
  var shouldAbort = false
  for (var i = 0; i < listeners.length; i++) {
    var result = listeners[i].call(this, entry)
    if (result === false) {
      shouldAbort = true
    }
  }
